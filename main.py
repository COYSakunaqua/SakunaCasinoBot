...
class SakunaBot(commands.Bot):
    def __init__(self):
        # 記憶體優化：僅開啟必要 Intents，減少 DisCloud RAM 占用
    ...
    async def setup_hook(self):
        ...
        print(f"✅ CasinOYS 核心啟動 | 動態 UI 模式已啟用", flush=True)
    ...
    async def auto_settle_task(self):
        ...
        # 預檢：若資料庫中無待結算 (status=0) 賽事，則跳過 API 請求以節省 Quota
        check = supabase.table("Events").select("event_id").eq("status", 0).execute()
        if not check.data:
            return

        print("🔍 [Auto-Settle] 正在獲取最新賽果...", flush=True)
...
class BetModal(discord.ui.Modal):
    def __init__(self, event_id, choice, odds, title):
        ...
        self.amt = discord.ui.TextInput(label=f"下注金額 (賠率: {odds})", placeholder="例如: 100", min_length=1)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            return await interaction.response.send_message("❌ 請輸入有效的正整數", ephemeral=True)
...
class BetView(discord.ui.View):
    def __init__(self, event_id, o_a, o_b, o_c, title, h_name, a_name):
        super().__init__(timeout=None) # 持久化視圖防止 Bot 重啟後按鈕失效
        self.eid, self.odds, self.title = event_id, {'A': o_a, 'B': o_b, 'C': o_c}, title
        
        # 動態更新按鈕文字：(隊伍名稱)win, draw
        self.b_a.label = f"{h_name} Win"
        self.b_b.label = "Draw"
        self.b_c.label = f"{a_name} Win"

    @discord.ui.button(label="Pending...", style=discord.ButtonStyle.green, custom_id="btn_a")
    async def b_a(self, i, b): 
        await i.response.send_modal(BetModal(self.eid, 'A', self.odds['A'], self.title))

    @discord.ui.button(label="Pending...", style=discord.ButtonStyle.gray, custom_id="btn_b")
    async def b_b(self, i, b): 
        await i.response.send_modal(BetModal(self.eid, 'B', self.odds['B'], self.title))

    @discord.ui.button(label="Pending...", style=discord.ButtonStyle.red, custom_id="btn_c")
...
@bot.tree.command(name="auto_open", description="【管理員】抓取歐聯賠率並開盤")
async def auto_open(interaction: discord.Interaction):
    ...
    try:
        ...
        for match in data[:3]:
            ...
            # 寫入 Events 表並獲取生成的 event_id [cite: 6]
            ...
            embed = discord.Embed(title="🏟️ CasinOYS 歐聯盤口", description=f"**{title}**", color=0x3498db)
        ...
        await interaction.followup.send("✅ 成功抓取最新賠率並開盤。")
    except Exception as e:
